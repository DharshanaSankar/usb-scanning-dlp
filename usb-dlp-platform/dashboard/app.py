...
def render_kpi_row(summary: dict, blocked_device_count: int) -> None:
    """Top-line counters: Total USB Events, Total Files Scanned, Total Alerts, Open Alerts, Blocked Transfers/Devices."""
    ...
    col4.metric("Open Alerts", summary.get("open_alerts", 0))
    col5.metric("Blocked Transfers", summary.get("blocked_transfers", 0))
    col6.metric("Blocked Devices", blocked_device_count)

    if blocked_device_count > 0:
        st.warning(
            f"🚫 {blocked_device_count} USB device(s) have had a transfer blocked. "
            f"See the **Blocked Devices** page (sidebar) for details."
        )
...
def render_recent_incidents(incidents: list) -> None:
    ...
    if "Severity" in df.columns:
        styler = df.style
        # pandas >= 2.1 renamed Styler.applymap() to Styler.map(); pandas 3.x
        # removed applymap entirely. Prefer map() and fall back to applymap()
        # for older pandas installations, so this works across versions.
        if hasattr(styler, "map"):
            styled = styler.map(_highlight_severity, subset=["Severity"])
        else:
            styled = styler.applymap(_highlight_severity, subset=["Severity"])
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        st.divider()
        st.caption(
            "Navigate using the pages listed above (USB Devices, File Activities, "
            "Incidents, Blocked Devices, Risk Reports)."
        )

    summary = db.fetch_dashboard_summary()
    distribution = db.fetch_risk_distribution()
    incidents = rows_to_dicts(db.fetch_recent_incidents(limit=10))
    blocked_device_count = len(db.fetch_blocked_device_summary())

    render_kpi_row(summary, blocked_device_count)
    st.divider()
    render_risk_distribution(distribution)
    st.divider()
    render_recent_incidents(incidents)
